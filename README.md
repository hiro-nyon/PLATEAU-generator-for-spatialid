...
- `citygml2id.py` に `--fill-solid`、`--underground-wall-only`、`--underground-z-min`、`--underground-z-max`、`--progress` を追加しています。
- UndergroundBuilding 向けに、穴あき形状の充填、地下壁のみのボクセル化、LOD4 対応を含む voxelization 改良を入れています。
- xlink 解決の無駄な処理を減らし、PyVista/VTK の互換性調整、診断スクリプトと回帰テストを追加しています。
